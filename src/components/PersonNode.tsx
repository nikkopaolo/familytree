...
export const PersonNode = ({ data, selected }: NodeProps<Person>) => {
  const person = data;
  const ageLabel = calculateAge(person.birthDate, person.deathDate);
  const statusLabel = person.isAlive ? "Alive" : "Deceased";
  const statusTone = person.isAlive ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700";
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex flex-wrap gap-2 text-xs">
        <span className="chip rounded-full px-2 py-1 text-slate-600">{person.gender ?? "Unspecified"}</span>
