...
export const TabNav = ({ activeTab, onChange, showSuggestions = true }: TabNavProps) => {
  ...
  return (
    <div className="mx-auto mt-3 flex w-[calc(100%-32px)] max-w-none flex-wrap items-center gap-2 rounded-2xl border border-white/70 bg-white/60 px-3 py-2 shadow-sm backdrop-blur">
      {visibleTabs.map((tab) => (
        <button
          key={tab.id}
